	...
	Sint16 y2, Sint16 rad, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	Sint16 cx = 0;
	Sint16 cy = rad;
	Sint16 ocx = (Sint16) 0x7fff;
	Sint16 ocy = (Sint16) 0x7fff;
	Sint16 df = 1 - rad;
	Sint16 d_e = 3;
	Sint16 d_se = -2 * rad + 5;
	...
	if (dx == 0) {
		...
		*/
		if (draw_endpoint)
		{
			...
			if (dy > 0) {
				return (vlineRGBA(renderer, x1, (Sint16)yy0, (Sint16)(yy0+dy), r, g, b, a));
			} else {
				return (pixelRGBA(renderer, x1, y1, r, g, b, a));
			}
		}
		...
		*/
		if (draw_endpoint)
		{
			...
			if (dx > 0) {
				return (hlineRGBA(renderer, (Sint16)xx0, (Sint16)(xx0+(xdir*dx)), y1, r, g, b, a));
			} else {
				return (pixelRGBA(renderer, x1, y1, r, g, b, a));
			}
		}
	...
	if (dy > dx) {
		...
		*/
		...
		while (--dy) {
			...
			*/
			...
			result |= pixelRGBAWeight (renderer, (Sint16)xx0, (Sint16)yy0, r, g, b, a, 255 - wgt);
			result |= pixelRGBAWeight (renderer, (Sint16)x0pxdir, (Sint16)yy0, r, g, b, a, wgt);
		}
		...
		*/
		...
		while (--dx) {
			...
			*/
			wgt = (erracc >> intshift) & 255;
			result |= pixelRGBAWeight(renderer, (Sint16)xx0, (Sint16)yy0, r, g, b, a, (Sint16)(255 - wgt));
			result |= pixelRGBAWeight (renderer, (Sint16)xx0, (Sint16)y0p1, r, g, b, a, wgt);
		}
	}
	...
	*/
    ...
    while (deltaX <= deltaY)
    {
		  ...
		  if ((scrX != oldX && scrY == oldY) || (scrX != oldX && scrY != oldY)) {
			result |= _drawQuadrants(renderer, x, y, (Sint16)scrX, (Sint16)scrY, f);
			oldX = scrX;
			oldY = scrY;
		  }
    }
	...
	if (curY > 0) 
	{
		...
		while (curY > 0)
		{
		    ...
		    if ((scrX != oldX && scrY == oldY) || (scrX != oldX && scrY != oldY)) {
				oldY--;
				for (;oldY >= scrY; oldY--) {
					result |= _drawQuadrants(renderer, x, y, (Sint16)scrX, (Sint16)oldY, f);
					/* prevent overdraw */
					if (f) {
						oldY = scrY - 1;
					}
	...
	for (y = miny; (y <= maxy); y++) {
		...
		*/
		...
		for (i = 0; (i < ints); i += 2) {
			...
			result |= hline(renderer, (Sint16)xa, (Sint16)xb, (Sint16)y);
		}
	}
	...
	*/
	...
	for (y = miny; (y <= maxy); y++) {
		...
		for (i = 0; (i < ints); i += 2) {
			...
			xb = gfxPrimitivesPolyInts[i+1] - 1;
			xb = (xb >> 16) + ((xb & 32768) >> 15);
			result |= _HLineTextured(renderer, (Sint16)xa, (Sint16)xb, (Sint16)y, textureAsTexture, texture->w, texture->h, texture_dx, texture_dy);
		}
	}
...
int stringRGBA(SDL_Renderer * renderer, Sint16 x, Sint16 y, const char *s, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	while (*curchar && !result) {
		...
		{
		case 0:
			curx += (Sint16)charWidthLocal;
			break;
		case 2:
			curx -= (Sint16)charWidthLocal;
			break;
		case 1:
			cury += (Sint16)charHeightLocal;
			break;
		case 3:
			cury -= (Sint16)charHeightLocal;
			break;
...
int bezierRGBA(SDL_Renderer * renderer, const Sint16 * vx, const Sint16 * vy, int n, int s, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	*/
	...
	if ((x=(double *)malloc(sizeof(double)*((Uint64)n+1)))==NULL) {
		return(-1);
	}
	if ((y=(double *)malloc(sizeof(double)*((Uint64)n+1)))==NULL) {
		free(x);
		return(-1);
	}    
	for (i=0; i<n; i++) {
...
int thickLineRGBA(SDL_Renderer *renderer, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, Uint8 width, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	if ((x1 == x2) && (y1 == y2)) {
		wh = width / 2;
		return boxRGBA(renderer, x1 - (Sint16)wh, y1 - (Sint16)wh, x2 + width, y2 + width, r, g, b, a);		
	}
